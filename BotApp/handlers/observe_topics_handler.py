...
from BotApp.config import NEWS_API_KEY
from BotApp.database.services.topic.service import TopicService
...
async def observe_topics(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    topics = TopicService().get_topics_for_user(update.message.from_user.id)
    if not topics:
